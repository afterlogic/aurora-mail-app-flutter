...
import 'package:aurora_mail/database/app_database.dart' show EventDb;
...
extension ActivityMapper on EventDb{
  ActivityBase? toActivity(){
    if(!this.onceLoaded){
      return ActivityBase(uid: uid, updateStatus: updateStatus, userLocalId: userLocalId, synced: synced, calendarId: calendarId);
    }
    ...
    return Task( description: description,
        ...
        isPrivate: isPrivate,
        reminders:
        remindersString != null && remindersString!.isNotEmpty
            ? remindersString!
            .split(',')
...
class EventMapper {
  static Activity synchronise({required Map<String, dynamic> newData, required ActivityBase base}){
    if(base is Task || newData['type'] == 'VTODO'){
  ...
  static Task taskFromJson({required Map<String, dynamic> newData, required ActivityBase base}){
  ...
  static List<List<ActivityBase>> groupEventsByCalendarId(List<ActivityBase> models) {
