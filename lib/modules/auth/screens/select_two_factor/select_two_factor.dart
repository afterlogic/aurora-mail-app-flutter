  ...
  Widget build(BuildContext context) {
    return TwoFactorScene(
      ...
      button: Column(
        ...
        children: [
          Text(i18n(context, S.tfa_label_hint_security_options)),
          SizedBox(height: 20),
          if (widget.args.state.hasSecurityKey) ...[
            SizedBox(
              width: double.infinity,
              child: AMButton(
                child: Text(i18n(context, S.tfa_btn_use_security_key)),
                onPressed: () {
          ...
          if (widget.args.state.hasAuthenticatorApp) ...[
            SizedBox(
              width: double.infinity,
              child: AMButton(
                child: Text(i18n(context, S.tfa_btn_use_auth_app)),
                onPressed: () {
                  Navigator.pushNamedAndRemoveUntil(
                    context,
          ...
          if (widget.args.state.hasBackupCodes) ...[
            SizedBox(
              width: double.infinity,
              child: AMButton(
                child: Text(i18n(context, S.tfa_btn_use_backup_code)),
                onPressed: null,
              ),
            ),
          ],
