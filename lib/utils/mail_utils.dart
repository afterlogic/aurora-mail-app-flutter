    ...
    return "<!doctype html>" +
        ...
        <html lang="en">
          ...
          0.1,
            ...
            ${useCustomBuildStyles ? 'return document.querySelector(".input-area").innerHTML;' : 'return document.body.innerHTML;'}
            }
            window.setBodyContent = function (content) {
              ${useCustomBuildStyles ? 'document.querySelector(".input-area").innerHTML=content;' : 'document.body.innerHTML=content;'}
            }
            window.setPlain = function () {
              ${useCustomBuildStyles ? 'document.querySelector(".input-area").style.whiteSpace = "pre";' : 'document.body.style = "white-space: pre;";'}
            }
            
            window.setHtml = function () {
                ${useCustomBuildStyles ? 'document.querySelector(".input-area").style.whiteSpace = "";' : 'document.body.style = "";'}
            }
            </script>
          </head>
          <body>
            ${useCustomBuildStyles ? '<div class="input-area" contenteditable="true"' + (!isHtml ? ' style="white-space: pre;"' : '') + '>$text</div>' : '<div contenteditable="true"' + (!isHtml ? ' style="white-space: pre;"' : '') + '>$text</div>'}
          </body>
        </html>
        """;
  }
